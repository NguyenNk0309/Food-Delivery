...
const Order = () => {
    ...
    useEffect(() => {
        if (sort === 'time') {
            ...
            setItems((prev) => prev.reverse().sort((a, b) => a.totalPrice - b.totalPrice))
        }
    }, [sort])
    ...
    return isLoading ? (
        ...
        <div className='w-full h-full md:pl-12 flex flex-col gap-5'>
            ...
            <div className='grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 grid-cols-1 gap-'>
                {items.map((item) => (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        ...
                        <p className='self-start text-base font-semibold'>
                            Total Price: <span className='font-normal'> {item.totalPrice}đ</span>
                        </p>
